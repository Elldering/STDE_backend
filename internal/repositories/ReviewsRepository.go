...
func GetReviewsAll() ([]models.Reviews, error) {
	...
	query, err := db.DB.Query("SELECT id, auth_user_sender_id, auth_user_recipient_id, grade, comment, created_at FROM Reviews")
	if err != nil {
		return nil, err
	}
	defer query.Close()
...
func GetReviewsById(id int) (models.Reviews, error) {
	...
	query := db.DB.QueryRow("SELECT id, auth_user_sender_id, auth_user_recipient_id, grade, comment, created_at FROM Reviews WHERE id=$1", id)
	var review models.Reviews
...
func PostReviews(data models.Reviews) error {
	...
	_, err := db.DB.Exec(query, data.AuthUserSenderID, data.AuthUserRecipientID, data.Grade, data.Comment)
	return err
}
