...
func GetAllUserProfile() ([]models.UserProfile, error) {
	...
	rows, err := db.DB.Query("SELECT id, first_name, last_name, middle_name, rating, profile_image FROM user_profile")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GetUserProfileById(id int) (models.UserProfile, error) {
	...
	row := db.DB.QueryRow("SELECT id, first_name, last_name, middle_name, rating, profile_image FROM user_profile WHERE id=$1", id)
	var userProfile models.UserProfile
...
func PostUserProfile(agp models.UserProfile) error {
	...
	result, err := db.DB.Exec(query, agp.FirstName, agp.LastName, agp.MiddleName, agp.Rating, agp.ProfileImage)
	if err != nil {
...
func PutUserProfile(id int, agp models.UserProfile) error {
	...
	result, err := db.DB.Exec("UPDATE user_profile SET first_name=$2, last_name=$3, middle_name=$4, rating=$5, profile_image=$6 WHERE id=$1", id, agp.FirstName, agp.LastName, agp.MiddleName, agp.Rating, agp.ProfileImage)
	if err != nil {
...
func DeleteUserProfile(id int) error {
	...
	result, err := db.DB.Exec("DELETE FROM user_profile WHERE id=$1", id)
	if err != nil {
