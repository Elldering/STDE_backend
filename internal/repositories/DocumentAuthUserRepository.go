...
func GetDocumentAuthUsers() ([]models.DocumentAuthUser, error) {
	...
	rows, err := database.DB.Query("SELECT id, auth_user_id, user_document_id FROM document_auth_user")
	if err != nil {
...
func GetDocumentAuthUserById(id int64) (models.DocumentAuthUser, error) {
	...
	row := database.DB.QueryRow("SELECT id, auth_user_id, user_document_id FROM document_auth_user WHERE id = $1", id)
	var dau models.DocumentAuthUser
...
func PostDocumentAuthUser(dau models.DocumentAuthUser) error {
	...
	result, err := database.DB.Exec(query, dau.AuthUserID, dau.UserDocumentID)
	if err != nil {
...
func PutDocumentAuthUser(id int64, dau models.DocumentAuthUser) error {
	...
	result, err := database.DB.Exec(query, dau.AuthUserID, dau.UserDocumentID, id)
	if err != nil {
